...
if __name__ == "__main__":
    ...
    # system, data_size, snr, iinit, ieq, eqsym = "lorenz", "small", 30, 0, 0, "x"
    for system in systems_collection:
        for data_size in data_sizes:
            for snr in snrs:
                for ieq, eqsym in enumerate(systems_collection[system].state_vars):
                    ...
                    for iinit in range(n_data):
                        ...
                        results.loc[results_subset.orig_index, 'val_TE'] = results_subset.val_TE
                        # calculate nanmean of TE for each row
                        results.loc[results_subset.orig_index, 'val_TE_nanmean'] = results_subset.val_TE.apply(lambda x: np.nanmean(x))

                    del results_subset

    # save results with additional validation column
    results.to_csv(f"{path_in_gathered_results}validation_gathered_results_{exp_version}_ALL_withValTE.csv", sep='\t', index=False)

    # get the best expression based on TE_nanmean for each method, system, snr, data_size, eq
    results_best = results.loc[results.groupby(['method', 'system', 'snr', 'data_size', 'eq'])['val_TE_nanmean'].idxmin()].reset_index(drop=True)
    results_best.to_csv(f"{path_in_gathered_results}validation_gathered_results_{exp_version}_BEST.csv", sep='\t', index=False)
